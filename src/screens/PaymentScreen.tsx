...
// API endpoint: sẽ dùng api.post('/client/phi-giao-dich')
...
// Helper function to format VND with dots and round numbers
...
const PaymentScreen = () => {
  ...
  // Fee information from API
  const [feePercent, setFeePercent] = useState<number>(0); // phi_phan_tram
  const [feeDiscount, setFeeDiscount] = useState<number>(0); // phi_giam_phan_tram
  const [feePercentReal, setFeePercentReal] = useState<number>(0); // phi_phan_tram_real
  const [feeVND, setFeeVND] = useState<number>(0); // phi_vnd
  const [feeVNDReal, setFeeVNDReal] = useState<number>(0); // phi_vnd_real
  const [amountVND, setAmountVND] = useState<number>(0); // amount_vnd
  const [amountVNDReal, setAmountVNDReal] = useState<number>(0); // amount_vnd_real
  ...
  const initializeData = async () => {
    ...
    try {
      ...
      if (paymentInfo.type === 'buy') {
        ...
        // Keep order: banks first (for metadata), then accounts
        await fetchBanks();
  ...
  useFocusEffect(
    React.useCallback(() => {
      // Skip immediate re-fetch on first mount
    ...
    try {
      ...
      if (res?.data?.status) {
        const list: BankAccount[] = (res.data.data || []).map((b: any) => {
          ...
          // console.log(`Bank ${b.id_bank}: API logo=${b.logo}, bankMeta logo=${bankMeta?.logo}`);
          return {
            id: b.id,
            bank: b.bank_name || (bankMeta ? bankMeta.name : `Bank ${b.id_bank}`),
            code: bankMeta ? bankMeta.code : '',
            logo: b.logo || bankMeta?.logo, // Ưu tiên logo từ API response, fallback về banks lookup
            bankId: b.id_bank,
  ...
  const fetchExchangeRate = async () => {
    ...
    try {
      // Calculate USDT amount based on payment type
      const amountNum = parseFloat(String(paymentInfo.amount));
      let usdtAmount: number;
      
      if (paymentInfo.type === 'buy') {
        // For buy: amount is VND, need to estimate USDT (use current rate or fallback)
        usdtAmount = amountNum / (currentRate || FALLBACK_RATE);
      } else {
        // For sell: amount is already USDT
        usdtAmount = amountNum;
      }

      const payload = {
        usdt: usdtAmount,
        type: paymentInfo.type === 'buy' ? 1 : 2
      };

      const res = await api.post('/client/phi-giao-dich', payload);
      const data: any = res.data || {};

      console.log('API phi-giao-dich response:', data);
      console.log('payload', payload);

      // Update all fee information
      if (data.ty_gia && !isNaN(parseFloat(String(data.ty_gia)))) {
        setCurrentRate(parseFloat(String(data.ty_gia)));
      }
      
      if (data.phi_phan_tram !== undefined) {
        setFeePercent(parseFloat(String(data.phi_phan_tram)));
      }
      
      if (data.phi_giam_phan_tram !== undefined) {
        setFeeDiscount(parseFloat(String(data.phi_giam_phan_tram)));
      }
      
      if (data.phi_phan_tram_real !== undefined) {
        setFeePercentReal(parseFloat(String(data.phi_phan_tram_real)));
      }
      
      if (data.phi_vnd !== undefined) {
        setFeeVND(parseFloat(String(data.phi_vnd)));
      }
      
      if (data.phi_vnd_real !== undefined) {
        setFeeVNDReal(parseFloat(String(data.phi_vnd_real)));
      }
      
      if (data.amount_vnd !== undefined) {
        setAmountVND(parseFloat(String(data.amount_vnd)));
      }
      
      if (data.amount_vnd_real !== undefined) {
        setAmountVNDReal(parseFloat(String(data.amount_vnd_real)));
      }
    ...
    if (!isInitialLoading) {
      ...
      const timer = setInterval(() => {
        setSecondsLeft((prev) => {
          if (prev <= 1) {
            // time to refresh
            // Avoid overlapping fetch
            if (!isFetchingRef.current) fetchExchangeRate();
            return 60;
          }
          return prev - 1;
        });
  ...
  const getTransactionInfo = () => {
    ...
    if (paymentInfo.type === 'buy') {
      // Buy USDT: amount is VND
      const usdtAmount = (amountNum / currentRate).toFixed(2);
      const transactionId = transactionIdRef.current;

      return {
        usdtWant: `${usdtAmount} USDT`,
        exchangeRate: `1 USDT = ${formatVND(currentRate)} VND`,
        feePercent: feePercent,
        feeDiscount: feeDiscount,
        feePercentReal: feePercentReal,
        feeVNDReal: feeVNDReal,
        totalVND: `${formatVND(amountVNDReal)} VND`,
        transactionId,
        transferInfo: {
          bankName: 'BIDV',
          ...
          amount: formatVND(amountVNDReal),
        }
      };
    } else {
      // Sell USDT: amount is USDT
      return {
        ...
        feePercentReal: feePercentReal,
        feeVNDReal: feeVNDReal,
        receiveVND: `${formatVND(amountVNDReal)} VND`,
        sendAmount: `${paymentInfo.amount} USDT`,
        receiveAmount: `${formatVND(amountVNDReal)} VND`,
        total: `${formatVND(amountVNDReal)} VND`,
      };
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <TouchableOpacity style={styles.headerRight} onPress={fetchExchangeRate}>
          ...
          <Text style={styles.headerRightText}>
            {secondsLeft}s • {formatVND(currentRate)}
          </Text>
          {isLoadingRate && (
            ...
            <>
              ...
              <View style={styles.infoRow}>
                ...
                <Text style={styles.infoValue}>
                  {formatVND(feeVND)} VND ({transactionInfo.feePercent}%)
                </Text>
              </View>
              <View style={styles.infoRow}>
                <Text style={styles.infoLabel}>{t('payment.discount')}</Text>
                <Text style={[styles.infoValue, { color: '#4CAF50' }]}>
                  -{formatVND(feeVND - feeVNDReal)} VND (-{transactionInfo.feeDiscount}%)
                </Text>
              </View>
              <View style={styles.infoRow}>
                <Text style={styles.infoLabel}>{t('payment.actualFee')}</Text>
                <Text style={styles.infoValue}>
                  {formatVND(transactionInfo.feeVNDReal)} VND ({transactionInfo.feePercentReal}%)
                </Text>
              </View>
              ...
              <>
                <SelectCustom
                  ...
                  options={wallets.map((w) => ({
                    ...
                    value: String(w.id),
                    subtitle: w.address,
                    iconUrl: require("../assets/images/logo_polygon.png"),
      ...
      <LoadingOverlay 
        visible={isLoading} 
        message={paymentInfo.type === 'buy' ? t('payment.creatingBuyTransaction') : t('payment.creatingSellTransaction')} 
      />
      
      <LoadingOverlay 
        visible={isInitialLoading} 
        message={t('payment.loadingTransaction')} 
      />
    </SafeAreaView>
  );
};
