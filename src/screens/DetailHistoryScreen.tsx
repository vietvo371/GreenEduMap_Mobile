...
const DetailHistoryScreen = () => {
  ...
  const handleCancelTransaction = async () => {
    ...
    Alert.alert(
      "",
      t('detailHistory.cancelTransaction'),
      [
        { text: t('common.cancel'), style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              const res = await api.post('/client/transactions/cancel', {
                id_transaction: idTransaction,
              });
              console.log('res cancel transaction', res);
              if (res?.data?.status) {
                Alert.alert(t('common.success'), res?.data?.message || 'Transaction cancelled');
                (navigation as any).navigate('MainTabs', { screen: 'History' });
              } else {
                Alert.alert(t('common.error'), res?.data?.message || 'Cancel failed');
              }
            } catch (err: any) {
              Alert.alert(t('common.error'), err?.response?.data?.message || 'Cancel failed');
            } finally {
  ...
  React.useEffect(() => {
    ...
    const interval = setInterval(() => {
      if (transaction?.createdAt) {
        ...
        // Auto navigate to History when expired
        ...
        setSecondsLeft(prev => {
          const newSeconds = prev > 0 ? prev - 1 : 0;
          // Auto navigate to History when expired
          if (newSeconds <= 0) {
            (navigation as any).navigate('MainTabs', { screen: 'History' });
          }
  ...
  }, [transaction?.createdAt, navigation]);
        ...
        {isPending && (
          <View style={styles.actionContainer}>
            ...
            >
              ...
              <Text style={styles.cancelButtonText}>{isCancelling ? t('common.loading') : t('detailHistory.cancelTransactionbtn')}</Text>
            </TouchableOpacity>
          </View>
        )}
         {/* Note */}
